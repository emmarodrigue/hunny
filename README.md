...
- We are getting more used to using git and didn't have to rely on others as much help in that area as we did in the first few sprints.
- We are still having some issues with styling using Bootstrap on some of our pages, so not everything is rendered the same way.
- **Emma**
...
- We need to communicate more about what we're all looking for with new features and development since some people had disagreements about how some features were implemented.
- **Emma:** Create tests for the features that we have already implemented so that we can ensure no new developments will prevent parts of our features from working as expected. I already have experience using Selenium to automate test cases and think that my skills there would contribute more to the overall success of the project. I will add automated test cases to a test directory in our project to measure my improvement in this area.
